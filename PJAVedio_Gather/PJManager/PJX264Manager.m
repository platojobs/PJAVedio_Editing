...
@implementation PJX264Manager

static PJX264Manager* _instance = nil;
        ...
        if (i264Nal > 0)//解析出帧来了
        {
            for (int i = 0,last = 0; i < i264Nal; i++)
            {
                ...
                else if (self->p264Nal[i].i_type == NAL_PPS)
                {
                    ...
                    [[PJRtmpManager pj_getInstance] pj_send_video_sps_pps:sps andSpsLength:sps_len andPPs:pps andPPsLength:pps_len];/*发送sps pps*/
                }
                else
                {
                    [[PJRtmpManager pj_getInstance] pj_send_rtmp_video:self->p264Nal[i].p_payload andLength:i_frame_size - last];/*发送普通帧*/
                    break;
                }
